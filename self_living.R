...
write_csv(tp_wide, '/Users/ryanhuang/Desktop/107-1/business statistics/happyliving/self_living.csv')
...
ggplot(data = tp_long[tp_long$answered_field == 't9_self_living3' | tp_long$answered_field == 't9_self_living6',]) +
  geom_mosaic(aes(weight = predicted, x = product(gender,livingstatus,answered_field,elderly2), fill=livingstatus, na.rm=TRUE,offset = 0.1)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
